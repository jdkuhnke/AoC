...
const initialStart = charMap.map((row, idx) => [idx, row.indexOf('S')]).reduce((a, b) => a[1] !== -1 ? a : b, [-1,-1]) as [number, number];
const end = charMap.map((row, idx) => [idx, row.indexOf('E')]).reduce((a, b) => a[1] !== -1 ? a : b, [-1,-1]) as [number, number];
const otherStarts = charMap.map((row, idx): [number, number] => [idx, row.indexOf('a')]);

const starts = [initialStart, ...otherStarts]
...
while (nodesToCheck.length > 0) {
    ...
    const neighbors = possibleNeighbors
        ...
        .filter(([ri, ci]) => currentNodeHeight - map[ri][ci] <= 1) as [number, number][];
...
const shortestTrail = starts.map(start => distances[start[0]][start[1]]).sort((a, b) => a-b)[0];
console.log({shortestTrail});